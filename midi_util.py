...
def read_one_midi_and_cut(filename, output, cut_length=16):
    ...
    for i in reversed(range(midi_length)):
        if notes[i].start < cut_start_time or notes[i].start > cut_end_time:
            del notes[i]
...
def pitch_augmentation(data, note_sets, shift=range(-3, 4)):
    """
    對音高做資料強化，允許音高變化範圍為-3~3個半音
    """
    datas = []
    for s in shift:
        d = data.copy()
        # 只允許音高變化在0~127之間
        assert all(np.less_equal(0, d[:, -1] + s)) and all(
            np.less_equal(d[:, -1] + s, 127)
        ), "Pitch augment caused out-of-range pitch"
        d[:, -1] += s
...
def read_midi_files(paths, valid_paths):
    ...
    def map_value_to_index(datass):
        ...
        for k in range(len(datass)):  # Loop over different folders
            ...
            datass[k] = np.array(datass[k]).astype(
                np.int64
